...
public sealed class PgUpVersionAttribute : Attribute
{
    ...
    public static PgUpVersionAttribute FromJson(string pgUpJson)
    {
        ...
        {
            throw PgUpExit.With("PgUp version is missing. Ensure that the version json element is present in the pgup.json file.");
        }
    ...
    public IPgUpProject Deserialize(string pgUpJson)
    {
        ...
        catch (JsonException e)
        {
            ...
            throw PgUpExit.With($"Invalid pgup.json file. {message} (path: {e.Path}. line: {e.LineNumber})");
        }
        catch (Exception e)
        {
            throw PgUpExit.With($"Failed to parse pgup.json file. {e.Message}");
        }
    }

...
public sealed class CliSubCommand_ctor_Should
{
        ...
        string expectedSubCommandPattern)
    {
        ...
        var target = new CliSubCommand(pattern);
    ...
    {
        var exception = Assert.Throws<ArgumentException>(() => new CliSubCommand(pattern));
        Assert.Contains(CliSubCommand.ArgumentExceptionMessage, exception.Message);
    }
}
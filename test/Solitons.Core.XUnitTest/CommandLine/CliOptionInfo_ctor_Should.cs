...
public sealed class CliOptionInfo_ctor_Should
{
    ...
    public void HandleIntValueProperty(int? defaultValue, string description, bool isRequired, int? value)
    {
        ...
        var target = new CliOptionInfo(metadata.Object, defaultValue, description, typeof(int))
        {
        ...
        var match = Regex.Match(input, $@"^(?<{target.RegexMatchGroupName}>\d+)?\S*$"); 
        if (match.Success && value.HasValue)
        {
            Assert.Equal(value, target.Deserialize(match, key => key)); 
        }
        else if (!match.Success && isRequired)
        {
